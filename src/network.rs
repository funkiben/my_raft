...
pub trait NetworkInterface<S: StateMachine> {
    ...
    fn handle_command_applied(&mut self, request: ClientCommandRequest<&S::Command>, state_machine: &S);

    fn handle_ready_to_read(&mut self, request: Self::ReadRequest, state_machine: &S);

    fn redirect_command_request(&mut self, leader_id: u32, request: ClientCommandRequest<S::Command>);

    fn redirect_read_request(&mut self, leader_id: u32, request: Self::ReadRequest);
}
...
// TODO rethink this
pub struct ClientCommandRequest<C> {
    pub request_id: u32,
    pub client_id: u32,
    pub command: C,
}